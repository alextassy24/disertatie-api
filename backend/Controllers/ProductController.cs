...
namespace backend.Controllers
{
                ...
                ) : ControllerBase
    {
        ...
        public async Task<IActionResult> RegisterProduct([FromBody] RegisterProductViewModel model)
        {
            ...
            var wearer = await _context.Wearers.FirstOrDefaultAsync(w => w.Id == model.WearerID && w.User == user);
            if (wearer is null)
            {
                return NotFound();
            }
        ...
        public async Task<IActionResult> DeleteProduct(int id)
        {
            ...
            var product = await _context.Products.FirstOrDefaultAsync(p => p.User == user && p.Id == id);
            if (product is null)
            {
                return NotFound();
            }
